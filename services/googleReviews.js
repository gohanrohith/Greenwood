...
async function syncGoogleReviews(placeId, apiKey, dbQuery) {
  ...
  for (const r of reviews) {
    ...
    const existing = await dbQuery('SELECT id FROM testimonials WHERE source_id=? AND source=?', [sourceId, 'google']);
    if (!existing || existing.length === 0) {
      await dbQuery(
        'INSERT INTO testimonials (name, role, quote, rating, source, source_id, avatar_url, is_active, sort_order) VALUES (?,?,?,?,?,?,?,1,50)',
        [r.author_name, 'Google Review', text, Math.round(r.rating || 5), 'google', sourceId, r.profile_photo_url || null]
      );
      synced++;
    }
  }
